		...
		<div class="container">
			<div class="row">
				<div class="col-md-5 text-left " style="">
					<h1 class="mb-3">Tabela Bibliotecilor</h1>
				</div>
			</div>
		</div>
	</div>
			...
			<div class="row">
				<div class="col-md-12">
					<div class="table-responsive">
						<form action="sterge_Biblioteca.jsp" method="post">
							<table class="table table-striped table-borderless">
								<thead>
									<tr>
										<th scope="col">Mark:</th>
										<th scope="col">IdBiblioteca:</th>
										<th scope="col">IdAutor:</th>
										<th scope="col">Nume Autor:</th>
										<th scope="col">Prenume:</th>
										<th scope="col">Data Nasterii:</th>
										<th scope="col">IdCarte:</th>
										<th scope="col">Nume Carte:</th>
										<th scope="col">Gen:</th>
										<th scope="col">Data Publicarii:</th>
										<th scope="col">Nume Biblioteca:</th>
										<th scope="col">Adresa Bibliotecii:</th>
									</tr>
								</thead>
								<%
                    jb.connect();
                    ResultSet rs = jb.vedeBiblioteca();
                    long x;
                    while (rs.next()) {
                        x = rs.getInt("idBiblioteca");
                %>
								<tbody>
									<tr>
										<td><input type="checkbox" name="primarykey"
											value="<%= x%>" /></td>
										<td><%= x%></td>
										<td><%= rs.getInt("idAutor_bibl")%></td>
										<td><%= rs.getString("NumeAutor")%></td>
										<td><%= rs.getString("Prenume")%></td>
										<td><%= rs.getString("DataNasterii")%></td>
										<td><%= rs.getInt("idCarte_bibl")%></td>
										<td><%= rs.getString("NumeCarte")%></td>
										<td><%= rs.getString("Gen")%></td>
										<td><%= rs.getString("DataPublicarii")%></td>
										<td><%= rs.getString("NumeBibl")%></td>
					...
					<input href="sterge_Biblioteca" type="submit"
						class="btn btn-lg btn-dark shadowed"
						value="Sterge liniile selectate"> <a
						class="btn btn-lg btn-dark shadowed" href="nou_Biblioteca.jsp">Adauga
						Biblioteca</a> <a class="btn btn-lg btn-dark shadowed"
						href="modifica_Biblioteca.jsp">Modifica Biblioteca</a>
				</div>
				</form>
				<%
            rs.close();
